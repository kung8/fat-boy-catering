                ...
                <div key={id} className="selection-container">
                    ...
                    <div className="selector-list">
                        {ingredients.map(item => {
                            const { ingredient_id: ingredientId, enabled, name: ingredientName, preset } = item;
                            if (enabled) {
                                if (selectionType === 1) {
                                    return (
                                        <div key={ingredientId} className={`ingredient-item radio-type align-ctr `}>
                                            <input className={`${preset && 'checked'}`} type="radio" name={id} id={ingredientId} />
                                    ...
                                    return (
                                        <label key={ingredientId} className="ingredient-item checkbox-type align-ctr" htmlFor={ingredientId}>
                                            <input className={`${preset && 'checked'}`} type="checkbox" name="checkbox" id={ingredientId} />
