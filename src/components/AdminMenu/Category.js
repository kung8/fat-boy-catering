...
export default function Category(props) {
    const { category, index, screenSize, mini, menuItemToggleFromAdmin, isLast, showToast } = props;
    ...
    const saveCategory = async (bool) => {
        ...
        if (check) {
            ...
            showToast('Please feel out all the fields', 'salmon')
            // toast where it will prompt to fill out the image and name
        }
    }
    ...
    return (
        <div className={`category-card col align-ctr ${collapsed && 'collapsed-card'} ${isLast && !collapsed && 'extra-spacing'}`}>
            ...
            <div className={`category-img-and-menu col wrap align-ctr ${collapsed && 'none'}`} style={{ height: !collapsed && getHeight() }}>
                ...
                {menuItems.length > 0 && displayMenuItem()}
                {typeof id === 'number'
                    &&
